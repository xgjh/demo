<template>
   <div id="pop">
          ...
          <div class="module-dialog clear module-dialog-address module-dialog-show" @click="hide($event)">
            <div class="dialog-panel" @click="hide($event)">
                <div class="topbar">
                    <div class="dialog-tit clear">
                        <h4 class="js-dialog-title">管理收货地址</h4>
                    </div>
                    <span class="dialog-close" @click="hide($event)">x</span>
                </div>
                                ...
                                <div class="address-form">
                                    ...
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 select-item province-wrapper">
                                            <select @change="selectFn('1')" v-model="locIndex1" name="province_code" class="province select-province js-form-province js-verify">
                                                <option value="-1">请选择省份</option>
                                                <option v-for="(item,index) in loc1" :key="index" 
                                                :value="index">{{item.area_name}}</option>
                                    ...
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                                            <select @change="selectFn('2')"  v-model="locIndex2" class="city select-city js-form-city js-verify">
                                                <option value="-1">请选择城市</option>
                                                <option v-for="(item,index) in loc2" :key="index" 
                                                :value="index">{{item.area_name}}</option>
                                            </select>
                                        </div>
                                        <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                                            <select class="city select-city js-form-city js-verify">
                                                <option value="-1">请选择区县</option>
                                                 <option v-for="(item,index) in loc3" :key="index" 
                                                :value="index">{{item.area_name}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="dialog-blue-btn big-main-btn disabled-btn js-verify-address">
                                        <a class="submit" @click="hide($event)">保存</a>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
import location from '../assets/js/addLocation'
    export default {
        data() {
            return {
                loc1: location,
        ...
        computed: {
            loc2() {
                var loc2=(this.locIndex1==-1)?[]:this.loc1[this.locIndex1].city_list
                return loc2;
            },
            loc3(){
                var loc3=(this.locIndex2==-1)?[]:this.loc2[this.locIndex2].region;
                return loc3;
            }
        },
        methods: {
            hide(e) {
                if(e.currentTarget.className!='dialog-panel'){
                    this.$emit('hide'); 
                }
                e.stopPropagation();  
            },
            selectFn(n){
                if(n==1){
                    this.locIndex2=this.locIndex3=-1;
                }else{
                    this.locIndex3=-1;
                }
