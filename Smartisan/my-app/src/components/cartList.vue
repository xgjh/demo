			...
			<div class="store-content">
				<div class="cart-box">
					...
					<div class="cart-inner">
						<div class="empty-label" :class="[cartList.length==0?'':'hide']">
							<h3>您的购物车中还没有商品</h3>
						...
						<div v-if="cartList.length!=0">
							...
							<div class="cart-table">
								<div class="cart-group">
									<!--购物列表-->
									<div class="cart-top-items" v-for="(item,index) in cartList" :key="index" >
										<div class="cart-items">
											<div class="items-choose" @click="changeSelect(index)">
												<span class="blue-checkbox-new" :class="[item.isSelect?'checkbox-on':'']"><a></a></span>
											</div>
											...
											<div class="operation">
												<a class="items-delete-btn" @click="del(index)" ></a>
											</div>
											<div class="subtotal">¥ {{item.price*item.num}}</div>
											<div class="item-cols-num">
												<div class="select js-select-quantity">
													<span class="down" :class="[item.num==0?'down-disabled':'']" @click="thin(index)">-</span>
													<span class="num">
														<input type="text" style="display: inline-block;" :value="item.num">
														<ul>
															<li>1</li>
															<li>2</li>
														</ul>
													</span>
													<span class="up" :class="[item.num==item.limit_num?'up-disabled':'']" @click="add(index)">+</span>
													<prompt v-if="item.num==item.limit_num"></prompt>
												</div>
											</div>
						...
						<div class="cart-bar-operation">
							<div>
								<div class="choose-all js-choose-all">
									<span class="blue-checkbox-new" :class="{'checkbox-on':isAllSelect}" @click="changeAll(isAllSelect)"><a></a></span>
									全选
								</div>
						...
						<div class="shipping">
							<div class="shipping-box">
								<div class="shipping-total shipping-num">
									<h4 class="">
										已选择 <i>{{allinfo.selectNum}}</i> 件商品
									</h4>
									<h5>
										共计 <i >{{allinfo.all}}</i> 件商品
									</h5>
								</div>
								<div class="shipping-total shipping-price">
									<h4 class="">
										应付总额：<span>￥</span><i >{{allinfo.selectPrice}}</i> 
									</h4>
							...
							<span class="jianguo-blue-main-btn big-main-btn js-checkout" :class="[allinfo.selectNum==0?'disabled-btn':'']">
...
import prompt from './prompt.vue'
    export default {
       ...
       methods: {
           ...
           changeAll(sta){
               this.$store.commit('changeAll',sta);
           }
